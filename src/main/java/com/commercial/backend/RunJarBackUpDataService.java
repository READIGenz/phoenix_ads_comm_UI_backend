...
public class RunJarBackUpDataService {
    @Autowired
    ...
    public String runJar() throws SQLException, IOException {
        ...
        dataConversionService.executeProcedure(commercialConstants.getDuplicateTablesProcedure(),commercialConstants.getDuplicateTableSqlPath());
        dataConversionService.executeProcedure(commercialConstants.getDropBackupProcedure(),commercialConstants.getDropBackupPath());
        ...
        try {
            ...
            try (InputStream input = new FileInputStream(new File(configFolderPath, commercialConstants.getApplicationProperties()))) {
                ...
                e.printStackTrace();
                return commercialConstants.getErrorLoadingConfiguration() + e.getMessage();
            }
            ...
            try (InputStream input = new FileInputStream(new File(configFolderPath, commercialConstants.getHeadProperties()))) {
                ...
                // Retrieve the value for "header.seg.cycle.date.reported"
                reportedDate = headSegProperties.getProperty(commercialConstants.getHeaderSegCycleDateReported());
            } catch (IOException e) {
                e.printStackTrace();
                return commercialConstants.getErrorLoadingHeadsegProperties() + e.getMessage();
            }

            dataConversionService.runDuplicateProcedure(reportedDate);
            dataConversionService.runProcedure(commercialConstants.getDropBackupProcedure());
            ...
            if (exitCode == 0) {
                String message = "Success! Your file for submission to the CICs has been generated. "
                        + "Backup is created in the Database. "
            ...
            e.printStackTrace();
            return commercialConstants.getErrorExecutingJar() + e.getMessage();
        }
    }
}